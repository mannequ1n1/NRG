program package_interface
    ...
    use flame_stabilization_control_class
    ...
    do task2 = 1, 1
        ...
        work_dir = 'DGun_fill_acetylene_2D_axisymmetric'
        ...
        ! Axisymmetric (cylindrical) coordinate system: x -> radial direction (r), y -> axial direction (z)
        ! Inlet is centered on the axis of symmetry (x=0, r=0)
        ...
        ! No flame stabilization needed for gas injection problem
        problem_flame_stabilization = flame_stabilization_control_c(mode = 'none')
        problem_controls_setup = problem_controls_c(flame_stabilization = problem_flame_stabilization)
        ...
        problem_data_manager  = data_manager_c(problem_domain, problem_mpi_support, problem_chemistry, problem_thermophysics, problem_solver_options, problem_controls_setup)
        ...
        !****************************** Setting boundary conditions ************************************
            ...
            ! In cylindrical coordinates with x as radial direction:
            ! - y=0 and y=max are walls (top and bottom boundaries)
            ! - x=0 is the axis of symmetry with central inlet
            ! - x=max is the outlet
            problem_boundaries%bc_markers(:,utter_loop(2,1),:)  = 3 
            problem_boundaries%bc_markers(:,utter_loop(2,2),:)  = 3
            ! x=0 boundary (left): axis of symmetry with central inlet
            problem_boundaries%bc_markers(utter_loop(1,1),:,:)  = 3 
            ! x=max boundary (right): outlet
            problem_boundaries%bc_markers(utter_loop(1,2),:,:)  = 1 

            ! For axisymmetric setup: inlet at x=0 (left boundary), centered on the symmetry axis (r=0)
            ! In cylindrical coordinates with x as radial direction, the axis of symmetry is at x=0
            do j = utter_loop(2,1), utter_loop(2,2)
                if (j <= nint(inflow_radius/delta_x)) then
                    ! Set inlet boundary type on the axis region
        ...
        !****************************** Writing problem short description ******************************
            ...
            write(log_unit,'(A)') 'General description: Numerical modeling of gas injection in axisymmetric (cylindrical) configuration.'
            write(log_unit,'(A)') 'Main aim: To model gas fill dynamics using CABARET solver in axisymmetric geometry.' 
            write(log_unit,'(A)') '               Provide clear interpretation of the main stages of filling process.'
            write(log_unit,'(A)') '               Analyze geometrical factor effect on the flow evolution (axisymmetric vs cartesian).'
            write(log_unit,'(A)') 'Problem setup: Gas injection through circular inlet at the axis of symmetry in cylindrical domain.'
            write(log_unit,'(A)') '               Domain: x (radial, r) from 0 to L_r, y (axial, z) from 0 to L_z.'
            write(log_unit,'(A)') '               Inlet: centered at r=0 (symmetry axis) on the left boundary (x=0).'
            write(log_unit,'(A)') 'Solver setup: CABARET solver, gravity enabled in axial direction, cylindrical (axisymmetric) geometry.'
            write(log_unit,'(A)') 'Validation and comparison: Comparison with cartesian 2D results.'
            write(log_unit,'(A)') '--------------------------------------------------------------------------'
