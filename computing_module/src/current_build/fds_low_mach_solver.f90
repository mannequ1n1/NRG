...
!===============================================================================
    ...
    use flame_stabilization_solver_class
    ...
    type :: fds_solver
        ...
        type(flame_stabilization_solver) :: stabilization_solver
...
contains
    ...
    type(fds_solver) function constructor(manager,problem_data_io)
        ...
        ! Problem-specific active sources are concrete reusable subsolvers.  They
        ! always create their production fields; disabled controls leave them zero.
        ...
        call manager%get_cons_field_pointer_by_name( &
            scal_ptr, vect_ptr, tens_ptr, 'energy_production_ignition')
        constructor%E_f_prod_ignition%s_ptr => scal_ptr%s_ptr
        ...
        call manager%get_cons_field_pointer_by_name( &
            scal_ptr, vect_ptr, tens_ptr, 'velocity_production_turbulence')
        constructor%v_prod_turbulence%v_ptr => vect_ptr%v_ptr
        ...
        constructor%stabilization_solver = flame_stabilization_solver_c( &
            manager, constructor%load_counter, constructor%inlet_velocity)
    ...
    subroutine solve_problem(this,iteration,stop_flag)
        ...
        if (this%energy_ignition_flag) then
            call this%ignition_solver%solve(this%time-this%time_step, this%time_step)
        end if
        if (this%turbulence_forcing_flag) then
            call this%turbulence_solver%solve(this%time-this%time_step, this%time_step)
        end if
        ...
        associate (    div_v_int            => this%div_v_int%s_ptr        , &
            ...
            do i = cons_inner_loop(1,1),cons_inner_loop(1,2)
                if(bc%bc_markers(i,j,k) == 0) then
                    ...
                    if (this%energy_ignition_flag) div_v_int%cells(i,j,k) = div_v_int%cells(i,j,k) + &
						...
						if (predictor) then
                            ...
                            if (this%turbulence_forcing_flag) then
                                F_a%cells(dim,i,j,k) = F_a%cells(dim,i,j,k) - 0.5_dp * ( &
                                    ...
                                    this%v_prod_turbulence%v_ptr%pr(dim)%cells( &
